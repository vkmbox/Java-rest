...
Для сборки проекта используется maven, для выполнения сборки в корневом каталоге выполнить команду:
  mvn clean package
Запуск проекта можно выполнить командой в корневом каталоге:
  mvn spring-boot:run
Также исполняемый jar - файл можно запустить стандартным образом, например:
